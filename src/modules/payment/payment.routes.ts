...
router.get(
  ...
  paymentController.getAllByTenant.bind(paymentController)
);

router.get(
  '/summary',
  roleMiddleware(ROLES.LANDLORD),
  paymentController.getSummary.bind(paymentController)
);
...
router.get(
  '/',
  roleMiddleware(ROLES.LANDLORD),
  paymentController.getAllByLandlord.bind(paymentController)
);

// dynamic routes تيجي بعد كده
router.get(
  '/:id',
